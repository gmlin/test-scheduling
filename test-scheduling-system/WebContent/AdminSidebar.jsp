<a href="#"><strong>
		Administrator</strong></a>

<hr>

<ul class="nav nav-stacked">
	<li><a href="EditTestingCenter.jsp">Edit Testing Center Info</a></li>
	<li><a href="AddClosedDates.jsp">Add Closed Dates and Reserved Periods</a>
	<li><a href="ImportData.jsp">Import Data</a></li>
	<li><a href="Utilization.jsp">Display Utilization</a></li>
	<li><a href="PendingRequests.jsp">View Requests</a></li>
	<li><a href="AdminAppointment.jsp">Make Appointment</a></li>
	<li><a href="AdminViewAppointments.jsp">View Appointments</a></li>
	<li><a href="CheckIn.jsp">Check-In Student</a></li>
	<li><a href="GenerateReports.jsp">Generate Report</a></li>
</ul>

<hr>